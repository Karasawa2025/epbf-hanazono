# eBPF Process & File Hider Demo

利用 eBPF 技术隐藏 Linux 进程和文件的概念验证（PoC）程序。通过 hook `getdents64` 系统调用，在 `ps`、`top`、`ls` 等工具的输出中隐藏指定进程和文件。
...
| 功能 | 说明 |
|------|------|
| **进程隐藏** | 从 `ps`、`top`、`ls /proc/` 中隐藏指定 PID |
| **文件隐藏** | 从 `ls`、`find`、`readdir()` 中隐藏指定文件名 |
| **自身隐藏** | 可同时隐藏自身进程 + 自身二进制文件 |
| **多文件** | 支持同时隐藏多个文件（`-hide-file` 可重复使用） |
...
2. **sys_exit_getdents64**: 遍历用户态 `linux_dirent64` 数组
   - **PID 匹配**: PID 字符串编码为 u64 + mask，单次比较
   - **文件名匹配**: 文件名首 8 字节编码为 u64，在 hash map 中查找
3. **隐藏机制**: 通过 ringbuf 通知用户态，用户态打开 `/proc/<caller_pid>/mem` 修改前一条目的 `d_reclen` 使其跳过目标条目
4. **性能优化**: u64 编码 + hash map 查找，单次循环同时检测 PID 和文件名
...
# 隐藏多个文件
sudo ./ebpf-hide-proc -hide-file secret.txt -hide-file config.dat

# 组合：隐藏进程 + 多个文件
sudo ./ebpf-hide-proc -hide-self-file -hide-file secret.txt -hide-file data.db
```
...
find /path/to/dir/ -name "secret.txt"  # 找不到

# 直接访问仍有效
cat /proc/<PID>/cmdline       # 仍可读
cat /path/to/secret.txt       # 仍可读（知道路径就能访问）
...
```
=== eBPF Process & File Hider Demo ===
Target PID: 12266 (12266)
Hide files: [ebpf-hide-proc]

--- ps aux ---     HIDDEN!
--- ps -ef ---     HIDDEN!
--- ls output ---  ebpf-hide-proc HIDDEN!

--- Direct access ---
Name:   sleep
State:  S (sleeping)
cat ebpf-hide-proc: still accessible by path

--- Hider events ---
[hide-pid  #1] Removed '12266' from PID 12276's readdir (offset=3736)
[hide-file #1] Removed 'ebpf-hide-proc' from PID 12280's readdir (offset=128)
```

## 局限性

1. **进程隐藏**: 对 `ls /proc/` 的隐藏效果依赖时序（ringbuf → 用户态 → 写 mem 存在微秒级延迟）
2. **文件名长度**: 文件名 > 7 字符时使用前缀匹配 + 用户态验证
3. **需要 root 权限**: 加载 eBPF + 写入 `/proc/pid/mem`
4. **依赖 BTF**: 内核需编译 `CONFIG_DEBUG_INFO_BTF=y`
5. **直接访问有效**: 知道文件路径仍可 `cat`/`stat`，只影响目录列表

## 对抗检测

此隐藏方式可被以下方法检测:

- 直接 `stat("/path/to/hidden-file")` 或 `open()` 访问
